...
async function githubRequest(
  ...
  const t = token ?? TOKEN;
  if (!t) throw new Error("GITOWN_GITHUB_TOKEN no está configurado.");
...
async function commitToGitown(args: {
  ...
  if (!token) throw new Error("Se requiere github_token o GITOWN_GITHUB_TOKEN.");
  ...
  const commitSha = result.commit.sha as string;
...
async function getTownStatus(): Promise<string> {
  ...
  const lines = [
    ...
    ...sorted.slice(0, 10).map(
      ([name, count], i) =>
        `  ${i + 1}. ${name.padEnd(20)} ${count} commits → ${getTier(count)}`,
    ),
  ];
  ...
  lines.push(
    ``,
    `🌐 Ver el town: https://${OWNER}.github.io/${REPO}`,
  );
      ...
      inputSchema: {
        ...
        properties: {
          ...
          github_token: {
            type: "string",
            description:
              "GitHub PAT con permiso 'repo'. Opcional si GITOWN_GITHUB_TOKEN está en el entorno.",
          },
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "commit_to_gitown") {
      result = await commitToGitown(args as any);
